...
namespace CupheadMusicPlayer.Engine.Memory
{
    ...
    public struct MemInfo
    {
        ...
        public uint Protect;
        public uint Type;
        public override string ToString() => $"{BaseAddress} {Protect:X} {State:X} {Type:X} {RegionSize:X}";
    }
    ...
    public static class MemoryReader
    {
        ...
        public static string ReadAscii(this Process targetProcess, IntPtr address)
        {
            ...
            do
            {
                ...
                while (i < bytesRead)
                {
                    byte d = data[i++];
                    if (d == 0) { i--; break; }
                    else if (d > 127) { invalid = true; break; }
                }

                if (i > 0) sb.Append(Encoding.ASCII.GetString(data, 0, i));
                if (i < bytesRead || invalid) break;
                offset += 128;
            } while (bytesRead > 0);
        ...
        private static int OffsetAddress(this Process targetProcess, ref IntPtr address, params int[] offsets)
        {
            ...
            for (int i = 0; i < offsets.Length - 1; i++)
            {
                ...
                if (is64Bit) address = (IntPtr)BitConverter.ToUInt64(buffer, 0);
                else address = (IntPtr)BitConverter.ToUInt32(buffer, 0);
        ...
        public static Module64 Module64(this Process p, string moduleName)
        {
            ...
            if (modules != null)
            {
                for (int i = 0; i < modules.Length; i++)
                {
                    ...
                    if (module.Name.Equals(moduleName, StringComparison.OrdinalIgnoreCase)) return module;
                }
            }
            return null;
        }
        ...
        public static Module64[] Modules64(this Process p)
        {
            lock (ModuleCache)
            {
                ...
                if (!WinAPI.EnumProcessModulesEx(p.Handle, buffer, cb, out uint totalModules, 3u)) return null;
                ...
                {
                    stringBuilder.Clear();
                    if (WinAPI.GetModuleFileNameEx(p.Handle, buffer[count], stringBuilder, (uint)stringBuilder.Capacity) == 0u) return list.ToArray();
                    string fileName = stringBuilder.ToString();
                    stringBuilder.Clear();
                    if (WinAPI.GetModuleBaseName(p.Handle, buffer[count], stringBuilder, (uint)stringBuilder.Capacity) == 0u) return list.ToArray();
                    string moduleName = stringBuilder.ToString();
                    ModuleInfo modInfo = default(ModuleInfo);
                    if (!WinAPI.GetModuleInformation(p.Handle, buffer[count], out modInfo, (uint)Marshal.SizeOf(modInfo))) return list.ToArray();
